...
TASK='/home/shuhao/semantic-mr-pilot-20260908'
...
with socket.socket() as sock:sock.bind(('127.0.0.1',19491))
service=TASK+'/service';model=TASK+'/Qwen2.5-7B-Instruct'
subprocess.run(docker+['exec','--user','0:0',NAME,'mkdir',service],check=True)
args=['vllm','serve',model,'--served-model-name','Qwen2.5-7B-Instruct','--host','127.0.0.1','--port','19491',
...
cmd+=[NAME,'/bin/sh','-c','exec '+shlex.join(args)+' > '+shlex.quote(service+'/server.log')+' 2>&1']
...
print(json.dumps({'container_id':r['Id'],'server_log':service+'/server.log','port':19491}))
